...
contract DoNotBuyCoin is Context, IERC20, Ownable {
    ...
    string private _name = "Do Not Buy Coin";
    string private _symbol = "DNB";
    uint8 private _decimals = 8;

    uint256 public _taxFee = 4;
    uint256 private _previousTaxFee = _taxFee;
        uint256 private  _Burn_FEE = 1;    // 1% BURN FEE

    uint256 public _liquidityFee = 2; // 3% for charity and 3% for liquidity
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 private _tBurnTotal;
address public _BurnWallet=address(0);
    uint256 public _maxTxAmount = 5000000 * 10**6 * 10**8;
    uint256 private minimumTokensBeforeSwap = 500000 * 10**6 * 10**8; 
   // address public Fees_Address=0x5C568A9db969E20fF65bE62F9ee0b8017E42780e;
 ...
 (,uint256 rTransferAmount,,,,,) = _getValues(tAmount);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint divisor= _liquidityFee.add(_taxFee);
        uint256 liquidityAmount = contractTokenBalance.mul(_liquidityFee).div(divisor);
        uint256 charityAmount = contractTokenBalance.sub(liquidityAmount);
        
        uint256 firstHalf = liquidityAmount.div(3);
    ...
    function _reflectFee(uint256 rFee, uint256 tBurn, uint256 tFee) private {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256,uint) {
    ...
    function calculateLBurnFee(uint256 _amount) private view returns (uint256) {
     ...
     function burn(uint tBurn) public{
     _rOwned[_BurnWallet] = _rOwned[_BurnWallet].add(tBurn);
     _tTotal=_tTotal.sub(tBurn);

        emit Transfer(address(0), address(0), tBurn);
